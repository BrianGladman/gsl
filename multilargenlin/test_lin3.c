...
static int
lin3_fdf (const int evaldf, const gsl_vector * x, void *params, void * work)
{
  int status;
  gsl_matrix_view J = gsl_matrix_view_array(lin3_J, lin3_N, lin3_P);
  gsl_vector_view f = gsl_vector_view_array(lin3_f, lin3_N);
  size_t i, j;

  gsl_matrix_set_zero(&J.matrix);

  gsl_vector_set(&f.vector, 0, -1.0);
  gsl_vector_set(&f.vector, lin3_N - 1, -1.0);
  ...
  status = test_accumulate(8, &J.matrix, &f.vector, work);

  return status;
}

static gsl_multilarge_function_fdf lin3_func =
{
  &lin3_fdf,
  lin3_P,
  NULL,
