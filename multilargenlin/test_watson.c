...
static int
watson_fdf (const int evaldf, const gsl_vector * x, void *params, void * work)
{
  int status;
  gsl_matrix_view J = gsl_matrix_view_array(watson_J, watson_N, watson_P);
  gsl_vector_view f = gsl_vector_view_array(watson_f, watson_N);
  ...
  for (i = 0; i < watson_N - 2; ++i)
    {
      ...
      gsl_vector_set (&f.vector, i, sum2 - sum1*sum1 - 1.0);
    }

  gsl_vector_set(&f.vector, watson_N - 2, x1);
  gsl_vector_set(&f.vector, watson_N - 1, x2 - x1*x1 - 1.0);
...
static gsl_multilarge_function_fdf watson_func =
{
  &watson_fdf,
  watson_P,
  NULL,
