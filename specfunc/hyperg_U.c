                    ...
                    gsl_sf_result * result)
{
  ...
  if(N <= 0) {
	...
	result->val  = sum * coeff_val;
	result->err  = fabs(sum) * coeff_err;
	result->err += 2.0 * GSL_DBL_EPSILON * (fabs(M)+1.0) * fabs(result->val);
	return GSL_SUCCESS;
      }
      else {
        result->val = 0.0;
...
hyperg_U_series(const double a, const double b, const double x, gsl_sf_result * result)
{
  ...
  if(fabs(1.0 + a - b) < SQRT_EPS) {
	...
	dchu += t;
      ...
      result->val = dchu;
      result->err = 2.0 * GSL_DBL_EPSILON * (i+1.0) * fabs(dchu);
	  ...
	  *ln_multiplier = 0.0;
	  ...
	  if(stat_e == GSL_SUCCESS) {
	    *ln_multiplier = 0.0;
	    ...
	    result->val = 1.0/Ua;
	    result->err = 2.0 * GSL_DBL_EPSILON * (fabs(a_target-a)+1.0) * fabs(result->val);
	    *ln_multiplier = lnpre;
	  }
	  return stat_CF1;
	}
