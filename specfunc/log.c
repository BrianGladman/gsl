...
/* Chebyshev expansion for log(1 + t/2), -1<t<1
 */
 ...
 */
static double lopxmx_data[30] = {
  -0.138672928390147820418835791217,
   0.035898384862245412945107316988,
  -0.071796769724490825890214633977,
   0.0128252576445603980588699182746,
  -0.00257738807143578123150243783557,
   0.00055248724185826110548585010907,
...
int gsl_sf_log_1plusx_impl(const double x, double * result)
{
  if(x <= -1.0) {
    ...
    *result = x * (1.0  - x/2.0*(1.0 - 2.0/3.0*x*(1.0 - 3.0/4.0*x)));
    return GSL_SUCCESS;
  }
  else if(fabs(x) < 0.5) {
    double t = 2.0*x;
...
int gsl_sf_log_1plusx_mx_impl(const double x, double * result)
{
  if(x <= -1.0) {
    ...
    *result = -x*x/2.0 *(1.0 - 2.0/3.0*x*(1.0 - 3.0/4.0*x*(1.0 - 4.0/5.0*x)));
    return GSL_SUCCESS;
  }
  else if(fabs(x) < 0.5) {
    double t = 2.0*x;
    *result = gsl_sf_cheb_eval(&lopxmx_cs, t);
    return GSL_SUCCESS;
  }
  else {
    *result = log(1.0 + x) - x;
