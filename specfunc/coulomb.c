...
CLeta(double L, double eta, gsl_sf_result * result)
{
  ...
  return gsl_sf_exp_err_impl(arg_val, arg_err, result);
}


int
gsl_sf_coulomb_CL_impl(double lam, double eta, gsl_sf_result * result)
{
  if(result == 0) {
    return GSL_EFAULT;
  }
...
gsl_sf_coulomb_CL_list(double lam_min, int kmax, double eta, double * cl)
{
  int k;
  gsl_sf_result cl_0;
  gsl_sf_coulomb_CL_impl(lam_min, eta, &cl_0);
  cl[0] = cl_0.val;
                  ...
                  gsl_sf_result * F, gsl_sf_result * G)
{
  ...
  while(m < max_iter) {
    ...
    if(m > 15) {
      ...
      if(   max_abs_dA/(max_abs_dA + abs_A) < 10.0*GSL_DBL_EPSILON
         && max_abs_dB/(max_abs_dB + abs_B) < 10.0*GSL_DBL_EPSILON
         ) break;
    }
  ...
  FA.err = A_sum * ClamA.err * pow_x * x + 2.0*GSL_DBL_EPSILON*fabs(FA.val);
  FB.val = B_sum * ClamB.val / pow_x;
  FB.err = B_sum * ClamB.err / pow_x + 2.0*GSL_DBL_EPSILON*fabs(FB.val);
                   ...
                   gsl_sf_result * F, gsl_sf_result * G)
{
  ...
  F->val = C0.val * u_sum;
  F->err = C0.err * fabs(u_sum) + 2.0*GSL_DBL_EPSILON*fabs(F->val);
  G->val = (v_sum + 2.0*eta*u_sum * log(2.0*x)) / C0.val;
  G->err = (fabs(v_sum) + fabs(2.0*eta*u_sum * log(2.0*x))) / fabs(C0.val) * fabs(C0.err/C0.val);
	     ...
	     double * exponent)
{
  ...
  gsl_sf_result ai;
  gsl_sf_result bi;
  gsl_sf_airy_Ai_scaled_impl(zeta_half*zeta_half, GSL_MODE_DEFAULT, &ai);
  gsl_sf_airy_Bi_scaled_impl(zeta_half*zeta_half, GSL_MODE_DEFAULT, &bi);
  F *= ai.val;
  G *= bi.val;
			    ...
			    double * exp_F, double * exp_G)
{
  ...
  if(x < 0.0 || lam_F <= -0.5 || lam_G <= -0.5) {
    ...
    *exp_G = 0.0;
    ...
    F->val  = F_lam_F;
    F->err  = 2.0 * CF1_count * GSL_DBL_EPSILON * fabs(F->val);
    Fp->val = Fp_lam_F;
    Fp->err = 2.0 * CF1_count * GSL_DBL_EPSILON * fabs(Fp->val);
    G->val  = G_lam_G;
    G->err  = 2.0 * CF1_count * GSL_DBL_EPSILON * fabs(G->val);
    Gp->val = Gp_lam_G;
    Gp->err = 2.0 * CF1_count * GSL_DBL_EPSILON * fabs(Gp->val);
    *exp_F = 0.0;
    *exp_G = 0.0;
    ...
    *exp_G = exp_lam_G;
    ...
    F->val  = F_lam_F;
    F->err  = 8.0*(CF1_count+F_recur_count+1)*GSL_DBL_EPSILON * fabs(F->val);

    Fp->val = Fp_lam_F;
    Fp->err = 8.0*(CF1_count+F_recur_count+1)*GSL_DBL_EPSILON * fabs(Fp->val);

    G->val  = G_lam_G;
    G->err  = 8.0*(CF1_count+G_recur_count+1)*GSL_DBL_EPSILON * fabs(G->val);

    Gp->val = Gp_lam_G;
    Gp->err = 8.0*(CF1_count+G_recur_count+1)*GSL_DBL_EPSILON * fabs(Gp->val);
			            ...
			            double * F_exp)
{
  ...
  if(x < 0.0 || lam_min < -0.5) {
    ...
    *F_exp = 0.0;
    ...
    int k;
    int stat_F = gsl_sf_coulomb_wave_F_array_impl(lam_min, kmax,
                                                  eta, x, 
                                                  fc_array,
                                                  F_exp);
