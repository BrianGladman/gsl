			   ...
			   gsl_integration_rule * q)
{
  ...
  size_t *ndin = workspace->level; /* temporarily use level as space for ndin */
  ...
  else if ((abserr0 <= tolerance && abserr0 != resabs0) || abserr0 == 0)
    {
      *result = result0;
      *abserr = abserr0;
  ...
  *abserr = GSL_DBL_MAX;
  ...
  do
	...
	{
	  if (fabs (r_i - area12) <= 1e-5 * fabs (area12)
	      && error12 >= 0.99 * e_i)
	    {
	      if (!extrapolate)
		{
