		      ...
		      double * result, double * abserr, double * neval)
{
  ...
  switch (key) 
    ...
    case GSL_INTEG_GAUSS15:
      gsl_integration_qk15 (f, a, b, result, abserr, defabs, resabs) ;
      break ;
    case GSL_INTEG_GAUSS21:
      gsl_integration_qk21 (f, a, b, result, abserr, defabs, resabs) ;
      break ;
    case GSL_INTEG_GAUSS31:
      gsl_integration_qk31 (f, a, b, result, abserr, defabs, resabs) ;
      break ;
    case GSL_INTEG_GAUSS41:
      gsl_integration_qk41 (f, a, b, result, abserr, defabs, resabs) ;
      break ;      
    case GSL_INTEG_GAUSS51:
      gsl_integration_qk51 (f, a, b, result, abserr, defabs, resabs) ;
      break ;      
    case GSL_INTEG_GAUSS61:
      gsl_integration_qk61 (f, a, b, result, abserr, defabs, resabs) ;
      break ;      
    }
  ...
  if (abserr <= 50 * DBL_EPSILION * defabs && (abserr > epsabs || abserr > epsrel*fabs(result)))
    {
      ier = 2 ;
    } ;
  if (limit == 1) 
  ...
  if (ier || ((abserr <= epsabs || abserr <= epsrel*fabs(result)) && abserr != resabs) || abserr == 0)
    {
      /* set up neval */
      return /* SUCCESS */ ;
    }
