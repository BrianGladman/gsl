...
static void *
impeuler_alloc (size_t dim)
{
  impeuler_state_t *state =
    (impeuler_state_t *) malloc (sizeof (impeuler_state_t));

  if (state == 0)
    {
      GSL_ERROR_NULL ("failed to allocate space for impeuler_state",
                      GSL_ENOMEM);
    }

  state->A = gsl_matrix_alloc (IMPEULER_STAGE, IMPEULER_STAGE);
  ...
  state->YZ = (double *) malloc (dim * IMPEULER_STAGE * sizeof (double));
  ...
  state->fYZ = (double *) malloc (dim * IMPEULER_STAGE * sizeof (double));
  ...
  state->esol = modnewton1_alloc (dim, IMPEULER_STAGE);
...
static int
impeuler_apply (void *vstate, size_t dim, double t, double h,
                ...
                const gsl_odeiv2_system * sys)
{
  ...
  impeuler_state_t *state = (impeuler_state_t *) vstate;
  ...
  {
    int s = rksubs (y_onestep, h, y, fYZ, b, IMPEULER_STAGE, dim);
  ...
  {
    int s = rksubs (ytmp, h / 2.0, y, fYZ, b, IMPEULER_STAGE, dim);
  ...
  {
    /* Note: impeuler returns y using the results from two half steps
       instead of the single step since the results are freely
       available and more precise.
     */

    int s = rksubs (y_twostep, h / 2.0, ytmp, fYZ, b, IMPEULER_STAGE, dim);
...
static int
impeuler_set_control (void *vstate, gsl_odeiv2_control * c)
{
  impeuler_state_t *state = (impeuler_state_t *) vstate;
...
static int
impeuler_reset (void *vstate, size_t dim)
{
  impeuler_state_t *state = (impeuler_state_t *) vstate;
...
static const gsl_odeiv2_step_type impeuler_type = {
  "impeuler",                   /* name */
  1,                            /* can use dydt_in? */
  1,                            /* gives exact dydt_out? */
  &impeuler_alloc,
  &impeuler_apply,
  &impeuler_set_control,
  &impeuler_reset,
  &impeuler_order,
  &impeuler_free
};

const gsl_odeiv2_step_type *gsl_odeiv2_step_impeuler = &impeuler_type;
