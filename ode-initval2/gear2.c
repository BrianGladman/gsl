...
typedef struct
{
  ...
  gsl_odeiv2_step *primer;       /* stepper to use for priming */
  double *yim1;                 /* y_{i-1}    */
  double *k;                    /* work space */
...
gear2_alloc (size_t dim)
{
  ...
  state->primed = 0;
  state->primer = gsl_odeiv2_step_alloc (gsl_odeiv2_step_rk4imp, dim);
...
gear2_step (double *y, gear2_state_t * state,
            ...
            const size_t dim, const gsl_odeiv2_system * sys)
{
  ...
  if (state->primed == 0 || t == state->t_primed || h != state->last_h)
    {
      ...
      status =
        gsl_odeiv2_step_apply (state->primer, t, h, y, yerr, dydt_in, dydt_out,
                              sys);
  ...
  else
    {
      ...
      for (i = 0; i < dim; i++)
        {
          yerr[i] = ODEIV_ERR_SAFETY * 0.5 * (y[i] - y_onestep[i]);
          yim1[i] = y0[i];
        }
...
gear2_free (void *vstate)
{
  ...
  free (state->y0_orig);
  free (state->y_onestep);
  gsl_odeiv2_step_free (state->primer);

  free (state);
}

static const gsl_odeiv2_step_type gear2_type = { "gear2",        /* name */
  1,                            /* can use dydt_in */
...
const gsl_odeiv2_step_type *gsl_odeiv2_step_gear2 = &gear2_type;
