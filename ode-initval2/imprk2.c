...
static void *
imprk2_alloc (size_t dim)
{
  imprk2_state_t *state = (imprk2_state_t *) malloc (sizeof (imprk2_state_t));

  if (state == 0)
    {
      GSL_ERROR_NULL ("failed to allocate space for imprk2_state",
                      GSL_ENOMEM);
    }

  state->A = gsl_matrix_alloc (IMPRK2_STAGE, IMPRK2_STAGE);
  ...
  state->YZ = (double *) malloc (dim * IMPRK2_STAGE * sizeof (double));
  ...
  state->fYZ = (double *) malloc (dim * IMPRK2_STAGE * sizeof (double));
  ...
  state->esol = modnewton1_alloc (dim, IMPRK2_STAGE);
...
static int
imprk2_apply (void *vstate, size_t dim, double t, double h,
              ...
              const gsl_odeiv2_system * sys)
{
  ...
  imprk2_state_t *state = (imprk2_state_t *) vstate;
  ...
  {
    int s = rksubs (y_onestep, h, y, fYZ, b, IMPRK2_STAGE, dim);
  ...
  {
    int s = rksubs (ytmp, h / 2.0, y, fYZ, b, IMPRK2_STAGE, dim);
  ...
  {
    /* Note: imprk2 returns y using the results from two half steps
       instead of the single step since the results are freely
       available and more precise.
     */

    int s = rksubs (y_twostep, h / 2.0, ytmp, fYZ, b, IMPRK2_STAGE, dim);
...
static int
imprk2_set_control (void *vstate, gsl_odeiv2_control * c)
{
  imprk2_state_t *state = (imprk2_state_t *) vstate;
...
static int
imprk2_reset (void *vstate, size_t dim)
{
  imprk2_state_t *state = (imprk2_state_t *) vstate;
...
static const gsl_odeiv2_step_type imprk2_type = {
  "imprk2",                     /* name */
  1,                            /* can use dydt_in? */
...
const gsl_odeiv2_step_type *gsl_odeiv2_step_imprk2 = &imprk2_type;
