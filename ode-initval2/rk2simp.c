               ...
               double dydt_out[], const gsl_odeiv2_system * sys)
{
  ...
  for (i = 0; i < dim; i++)
    {
      yerr[i] = ODEIV_ERR_SAFETY * 0.5 * (y[i] - y_onestep[i]) / 3.0;
    }
...
static const gsl_odeiv2_step_type rk2simp_type = {
  "rk2simp",                    /* name */
  0,                            /* can use dydt_in? */
...
const gsl_odeiv2_step_type *gsl_odeiv2_step_rk2simp = &rk2simp_type;
