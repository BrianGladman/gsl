...

{
  ...
  if (side == CblasLeft && uplo == CblasUpper && trans == CblasNoTrans) {
    ...
    for (i = n1; i > 0 && i--;) {
      if (nonunit) {
	const BASE Aii_real = REAL(A, lda * i + i);
	const BASE Aii_imag = conj * IMAG(A, lda * i + i);
	const BASE s = hypot(Aii_real, Aii_imag);
	...
	for (j = 0; j < n2; j++) {
      ...
      for (k = 0; k < i; k++) {
	const BASE Aki_real = REAL(A, k * lda + i);
	const BASE Aki_imag = conj * IMAG(A, k * lda + i);
	for (j = 0; j < n2; j++) {
	  const BASE Bij_real = REAL(B, ldb * i + j);
	  const BASE Bij_imag = IMAG(B, ldb * i + j);
	...
	const BASE Aik_imag = conj * IMAG(A, i * lda + k);
	for (j = 0; j < n2; j++) {
	  const BASE Bij_real = REAL(B, ldb * i + j);
	  const BASE Bij_imag = IMAG(B, ldb * i + j);
    ...
    for (i = 0; i < n1; i++) {
      for (j = 0; j < n2; j++) {
	if (nonunit) {
	  const BASE Ajj_real = REAL(A, lda * j + j);
	  const BASE Ajj_imag = conj * IMAG(A, lda * j + j);
	  const BASE s = hypot(Ajj_real, Ajj_imag);
...

	{
	  ...
	  for (k = j + 1; k < n2; k++) {
	    ...
	    const BASE Ajk_imag = conj * IMAG(A, j * lda + k);
...

	{
	  ...
	  const BASE Bij_imag = IMAG(B, ldb * i + j);
	  for (k = 0; k < j; k++) {
	    const BASE Akj_real = REAL(A, k * lda + j);
	    const BASE Akj_imag = conj * IMAG(A, k * lda + j);
