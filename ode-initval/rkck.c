...
static int  rkck_step(void * state, unsigned int dim, double t, double h, double y[], double yerr[], gsl_odeiv_function * dydt);
...
static
int
rkck_step(void * state, unsigned int dim, double t, double h, double y[], double yerr[], gsl_odeiv_function * dydt)
{
  gsl_odeiv_step_rkck_state * s = (gsl_odeiv_step_rkck_state * ) state;

  /* Cash-Karp constants */
  const double a[] = { 0.0, 0.0, 0.2, 0.3, 0.6, 1.0, 0.875 };
  const double b21 = 0.2;
  const double b3[] = { 0.0, 3.0/40.0, 9.0/40.0 };
  const double b4[] = { 0.0, 0.3, -0.9, 1.2 };
  const double b5[] = { 0.0, -11.0/54.0, 2.5, -70.0/27.0, 35.0/27.0 };
  const double b6[] = { 0.0, 1631.0/55296.0, 175.0/512.0, 575.0/13824.0,44275.0/110592.0, 253.0/4096.0 };
  ...
  /* k2 step */
  status += ( GSL_ODEIV_FN_EVAL(dydt, t + a[2]*h, ytmp, k2) != 0 );
  for (i=0;i<dim;i++)
    ytmp[i] = y[i] + h*(b3[1]*k1[i] + b3[2]*k2[i]);

  /* k3 step */
  status += ( GSL_ODEIV_FN_EVAL(dydt, t + a[3]*h, ytmp, k3) != 0 );
  for (i=0;i<dim;i++)
    ytmp[i] = y[i] + h*(b4[1]*k1[i] + b4[2]*k2[i] + b4[3]*k3[i]);

  /* k4 step */
  status += ( GSL_ODEIV_FN_EVAL(dydt, t + a[4]*h, ytmp, k4) != 0 );
  for (i=0;i<dim;i++)
    ytmp[i] = y[i] + h*(b5[1]*k1[i] + b5[2]*k2[i] + b5[3]*k3[i] + b5[4]*k4[i]);

  /* k5 step */
  status += ( GSL_ODEIV_FN_EVAL(dydt, t + a[5]*h, ytmp, k5) != 0 );
  for (i=0;i<dim;i++)
    ytmp[i] = y[i] + h*(b6[1]*k1[i] + b6[2]*k2[i] + b6[3]*k3[i] + b6[4]*k4[i] + b6[5]*k5[i]);

  /* k6 step and final sum */
  status += ( GSL_ODEIV_FN_EVAL(dydt, t + a[6]*h, ytmp, k6) != 0 );
  for (i=0;i<dim;i++)
