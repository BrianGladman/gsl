  ...
  if(Uplo == CblasUpper) {
    for(j=0; j<N; j++) {
      BASE_TYPE tmp1R = REAL0(alpha) * REAL(Y, incY, j) - IMAG0(alpha) * IMAG(Y, incY, j);
      BASE_TYPE tmp1I = REAL0(alpha) * IMAG(Y, incY, j) + IMAG0(alpha) * REAL(Y, incY, j);
      BASE_TYPE tmp2R = REAL0(alpha) * REAL(X, incX, j) - IMAG0(alpha) * IMAG(X, incX, j);
      BASE_TYPE tmp2I = REAL0(alpha) * IMAG(X, incX, j) + IMAG0(alpha) * REAL(X, incX, j);
      for(i=j; i<N; i++) {
