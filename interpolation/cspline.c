...
typedef struct
{
  double * b; /* linear coefficients, length n */
  double * c; /* quadratic coefficients, length n */
  double * d; /* cubic coefficients, length n */
  double * g; /* right hand side, length n */
  double * diag;
  double * offdiag;
} cspline_state_t;

static void * cspline_alloc (size_t size);
static void cspline_free (void * vstate);
static int cspline_init (void * vstate, const double xa[], const double ya[], size_t size);

/* common initialization */
static void *
cspline_alloc (size_t size)
{
  cspline_state_t * state = (cspline_state_t *) calloc (1, sizeof (cspline_state_t));
 ...
 */
static int
cspline_init (void * vstate, const double xa[], const double ya[], size_t size)
{
  cspline_state_t *state = (cspline_state_t *) vstate;
  ...
  for (i = 0; i < size - 1; i++)
    {
      ...
      state->b[i] = (ya[i + 1] - ya[i]) / h_i - h_i / 3.0 * (2.0 * state->c[i] + state->c[i + 1]);
      state->d[i] = (state->c[i + 1] - state->c[i]) / (3.0 * h_i);
    }
              ...
              double *y)
{
  ...
  *y = y_lo + delx * (state->b[index] + delx * (state->c[index] + delx * state->d[index]));
                    ...
                    double *dydx)
{
  ...
  x_lo = x_array[index];
  delx = x - x_lo;
  *dydx = state->b[index] + delx * (2.0 * state->c[index] + 3.0 * state->d[index] * delx);
                     ...
                     double * y_pp)
{
  ...
  x_lo = x_array[index];
  delx = x - x_lo;
  *y_pp = 2.0 * state->c[index] + 6.0 * state->d[index] * delx;
                    ...
                    double * result)
{
  ...
  *result = 0.0;
  ...
  for(i=index_a; i<=index_b; i++) {
    ...
    if(dx != 0.0) {
      double b_i = state->b[i];
      double c_i = state->c[i];
      double d_i = state->d[i];
