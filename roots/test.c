...
main (void)
{
...

  {
    ...
    gsl_test_rel(x[0], 3.0, 1e-9, "x1, x^3 = 27");

    n = gsl_root_solve_cubic (-51.0, 867.0, -4913.0, x);

    gsl_test(n != 3, "gsl_root_solve_cubic, three roots, (x-17)^3=0") ;
    gsl_test_rel(x[0], 17.0, 1e-9, "x1, (x-17)^3=0");
    gsl_test_rel(x[1], 17.0, 1e-9, "x2, (x-17)^3=0");
    gsl_test_rel(x[2], 17.0, 1e-9, "x3, (x-17)^3=0");

    n = gsl_root_solve_cubic (-57.0, 1071.0, -6647.0, x);
    
    gsl_test(n != 3, "gsl_root_solve_cubic, three roots, (x-17)(x-17)(x-23)=0") ;
    gsl_test_rel(x[0], 17.0, 1e-9, "x1, (x-17)(x-17)(x-23)=0");
    gsl_test_rel(x[1], 17.0, 1e-9, "x2, (x-17)(x-17)(x-23)=0");
    gsl_test_rel(x[2], 23.0, 1e-9, "x3, (x-17)(x-17)(x-23)=0");
