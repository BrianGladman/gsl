...
int main (void) 
{
  ...
  status = 0 ;
  h = gsl_histogram2d_alloc(0,10) ;
  gsl_test(!status, "gsl_histogram_alloc traps zero-width histogram") ;
  gsl_test(h != 0, 
	   "gsl_histogram2d_alloc returns NULL for zero-width histogram") ;


  status = 0 ;
  h = gsl_histogram2d_alloc(10,0) ;
  gsl_test(!status, "gsl_histogram_alloc traps zero-length histogram") ;
  gsl_test(h != 0, 
	   "gsl_histogram2d_alloc returns NULL for zero-length histogram") ;


  status = 0 ;
  h = gsl_histogram2d_alloc_uniform (0, 10, 0.0, 1.0, 0.0, 1.0) ;
  gsl_test(!status, 
	   "gsl_histogram2d_alloc_uniform traps zero-width histogram") ;
  gsl_test(h != 0, 
	   "gsl_histogram2d_alloc_uniform returns NULL for zero-width histogram") ;

  status = 0 ;
  h = gsl_histogram2d_alloc_uniform (10, 0, 0.0, 1.0, 0.0, 1.0) ;
  gsl_test(!status, 
	   "gsl_histogram2d_alloc_uniform traps zero-length histogram") ;
  gsl_test(h != 0, 
	   "gsl_histogram2d_alloc_uniform returns NULL for zero-length histogram") ;

  status = 0 ;
  h = gsl_histogram2d_alloc_uniform (10, 10, 0.0, 1.0, 1.0, 1.0) ;
  gsl_test(!status, 
	   "gsl_histogram2d_alloc_uniform traps equal endpoints") ;
  gsl_test(h != 0, 
	   "gsl_histogram2d_alloc_uniform returns NULL for equal endpoints") ;


  status = 0 ;
  h = gsl_histogram2d_alloc_uniform (10, 10, 1.0, 1.0, 0.0, 1.0) ;
  gsl_test(!status, 
	   "gsl_histogram2d_alloc_uniform traps equal endpoints") ;
  gsl_test(h != 0, 
	   "gsl_histogram2d_alloc_uniform returns NULL for equal endpoints") ;
  ...
  status = 0 ;
  h = gsl_histogram2d_alloc_uniform (10, 10, 0.0, 1.0, 2.0, 1.0) ;
  gsl_test(!status, 
	   "gsl_histogram2d_alloc_uniform traps invalid range") ;
  gsl_test(h != 0, 
	   "gsl_histogram2d_alloc_uniform returns NULL for invalid range") ;


  status = 0 ;
  h = gsl_histogram2d_alloc_uniform (10, 10, 2.0, 1.0, 0.0, 1.0) ;
  gsl_test(!status, 
	   "gsl_histogram2d_alloc_uniform traps invalid range") ;
  gsl_test(h != 0, 
	   "gsl_histogram2d_alloc_uniform returns NULL for invalid range") ;
