...
\begin{document}
\title{FFT Algorithms}
\author{Brian Gough, {\tt bjg@vvv.lanl.gov}}
\date{May 1997}
\maketitle
...
results to convert a length-$p$ FFT (where $p$ is prime) into a
convolution of length-($p-1$).  There is a simple identity between the
convolution of length $N$ and the FFT of the same length, so if $p-1$
is easily factorizable this allows the convolution to be computed
efficiently via the FFT. The idea is presented in the original paper
by Rader~\cite{raderprimes} (also reprinted in~\cite{mcclellan79}),
but for more details see the theoretical books mentioned earlier.
\end{subsection}
...
\subsection{Details of the implementation}
%
First the function \verb\gsl_fft_complex_wavetable_alloc\ allocates
...
factorization function \verb\gsl_fft_factorize\ which takes a list of
...
\end{eqnarray}
%
The array \verb\twiddle[i]\ maintains a set of pointers into
\verb\trig\ for the starting points for the outer loop.  The core of
the implementation is \verb\gsl_fft_complex\. This function loops over
...
handed-off to a dedicated small-$N$ module, such as
\verb\gsl_fft_complex_pass_3\ or \verb\gsl_fft_complex_pass_5\.
Unimplemented factors are handled by the general-$N$ routine
\verb\gsl_fft_complex_pass_n\. The structure of one of the small-$N$
modules is a simple transcription of the basic algorithm given above.
Here is an example for \verb\gsl_fft_complex_pass_3\. For a pass with
a factor of 3 we have to calculate the following expression,
%
\begin{equation}
v'_{(a f + \lambda) p_{i-1} + b} 
= 
...
$\omega^a_{q_{i-1}}$ are taken out of the \verb\trig\ array.
